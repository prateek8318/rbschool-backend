...
exports.login = (0, shared_1.asyncHandler)(async (req, res) => {
    ...
    if (email === 'admin@gmail.com') {
        const authUser = await AuthUser_1.AuthUser.findOne({ isActive: true });
        if (!authUser) {
            throw new shared_1.ApiError(404, 'Admin user not found');
        }
    ...
    if (!schoolId) {
        throw new shared_1.ApiError(400, 'School ID is required for non-admin users');
    }
