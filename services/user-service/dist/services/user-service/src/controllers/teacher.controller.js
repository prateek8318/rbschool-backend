...
exports.getTeachers = (0, shared_1.asyncHandler)(async (req, res) => {
    ...
    const [teachers, total] = await Promise.all([
        TeacherProfile_1.TeacherProfile.find(query)
            .sort({ createdAt: -1 })
            .skip((page - 1) * limit)
            .limit(limit),
        TeacherProfile_1.TeacherProfile.countDocuments(query),
    ]);
    return shared_1.ApiResponse.success(res, 200, teachers, 'Teachers fetched successfully', { page, limit, total, totalPages: Math.ceil(total / limit) || 1 });
});
