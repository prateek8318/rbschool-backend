...
exports.getInternalSummary = exports.exportFeeReport = exports.getOverdueFees = exports.getFeeSummary = exports.recordPayment = exports.createFeeRecord = exports.getStudentFee = exports.getAllFees = void 0;
...
exports.exportFeeReport = (0, shared_1.asyncHandler)(async (req, res) => {
    const { role, schoolId } = req.user;
    if (role !== 'admin')
        throw new shared_1.ApiError(403, 'Only admin can export fee reports');
    const academicYear = String(req.query.academicYear ?? '');
    const format = String(req.query.format ?? 'csv').toLowerCase();
    if (!academicYear)
        throw new shared_1.ApiError(400, 'academicYear is required');
    if (format !== 'csv')
        throw new shared_1.ApiError(400, 'Only csv export is supported');
    const records = await FeeRecord_1.FeeRecord.find({ schoolId, academicYear, isArchived: false }).sort({ createdAt: -1 });
    const studentDetails = records.length
        ? (await axios_1.default.get(`${env_1.env.USER_SERVICE_URL}/internal/student-details`, {
            params: { ids: records.map((record) => record.studentId).join(',') },
        })).data.data
